...
> The attributes that decorate the generic parameters **must** implement the `IGenericParameterDecorator` interface.
> It serves as an empty marker distinguishing the attributes, chosen for this specific purpose, from others with different functions or those generated by the compiler.

If there are none or multiple attributes decorating a generic parameter, an exception will be thrown.
If the external configuration is missing some required mappings, an exception will be thrown as well.
