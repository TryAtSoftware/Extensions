...
{
    public static MemberInfo GetMemberInfo<T, TValue>(this Expression<Func<T, TValue>> propertyLambda)
    {
        if (propertyLambda is null) throw new ArgumentNullException(nameof(propertyLambda));

        MemberExpression? memberExpression;
        if (propertyLambda.Body is UnaryExpression body) memberExpression = body.Operand as MemberExpression;
        else memberExpression = propertyLambda.Body as MemberExpression;
